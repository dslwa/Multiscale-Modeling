...
import com.example.multiscaleca.seeding.RandomSeeder;
import com.example.multiscaleca.seeding.UniformSeeder;
import com.example.multiscaleca.seeding.ManualSeeder;
import com.example.multiscaleca.neighborhood.NeighborhoodStrategy;
import com.example.multiscaleca.neighborhood.VonNeumannStrategy;
import com.example.multiscaleca.neighborhood.PentagonalStrategy;
import com.example.multiscaleca.neighborhood.MooreStrategy;
import com.example.multiscaleca.neighborhood.RandomHexagonalStrategy;
...
public class MainApp extends Application {
    ...
    private void setupActions() {
        ...
        controlPanel.getGrowButton().setOnAction(e -> {
            TextField thresholdField = controlPanel.getDensityTargetField();
        ...
        controlPanel.getExportButton().setOnAction(e -> exportToOvito());
    }
    ...
    private void exportToOvito() {
        if (grid == null) return;

        int depth;
        try {
            depth = Integer.parseInt(controlPanel.getDepthField().getText());
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("output_ovito.xyz"))) {
            writer.write(Integer.toString(count));
            writer.newLine();
            writer.write("Generated by Grain Growth Simulator");  // komentarz
            writer.newLine();
            ...
            for (int z = 0; z < depth; z++) {
                for (int y = 0; y < H; y++) {
                    for (int x = 0; x < W; x++) {
                        var cell = grid.getCell(x, y);
                        if (cell != null && !cell.isEmpty()) {
                            writer.write(x + " " + y + " " + z + " " + cell.getGrainId());
                            writer.newLine();
                        }
                    }
                }
            ...
            System.out.println("Zapisano do output_ovito.xyz");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private void removeRandomGrains(int count) {
        ...
        for (var row : grid.getGrid()) {
            for (var cell : row) {
                if (cell.getGrainId() != 0) {
                    ids.add(cell.getGrainId());
                }
            }
        }
